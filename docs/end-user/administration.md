...
Hakken uses workspace, platform, and evidence-review roles. A standard `USER` can use the normal dashboard features available to their company. A company `ADMIN` can manage tenant-scoped organization areas and team members, subject to backend limits. A `SUPER_ADMIN` can manage the platform globally, create and configure companies, inspect global analytics, configure AI and maintenance systems, and impersonate a company workspace for support and setup. Read-only and auditor-style roles can inspect selected evidence surfaces, especially Governance, without receiving the same write controls as a super admin.

## Admin Navigation

When a super admin opens `/admin`, the sidebar shows platform administration groups. The dashboard item opens the global admin overview. Companies opens company management and company detail screens. AI contains the global AI overview, costs, money view, chat logs, diary, unanswered questions, rules, system prompt, Wiki, voice, widget, models, tools, and Skill Center. Agents contains agent management, workflow management, workflow runs, and schedules. Governance opens the AI register, policies, audit trail, and approval evidence. Settings contains system settings, plans, API keys, and analytics. Maintenance contains system health, external connection checks, scripts, and auth diagnostics. User Management contains the read-only platform user directory plus super-admin management and super-admin invitations.
...
From company detail screens, a super admin can review and configure the tenant. Current routes include overview, workspace feature gates, directory users and invitations, company calls, company mailbox, company AI prompt and rules, model defaults, knowledge source archive, Wiki pages/map/diary/unanswered questions, saved answers, checks, memory, skills, usage, chat logs, and widget configuration. Company prompts and rules influence how the assistant behaves for that tenant, but they do not override platform safety or tenant isolation.
...
The AI section centralizes governance of Hakken’s AI behavior. The costs and money screens show usage and financial reporting. Chat logs provide administrative visibility into conversations where supported by the current screen. Diary and unanswered-question screens show what the Wiki has learned and what users asked that it could not answer. Rules, system prompts, global Wiki and knowledge sources, company knowledge, model defaults, voice, tools, skills, and widget settings determine how the assistant behaves, what it can reference, and which engines are available.
...
The agents group contains advanced operational tools: agent management, workflow builders, workflow runs, and schedules. Approvals now sit in Governance because they are evidence and oversight decisions. These areas are more powerful than normal assistant chat because they can involve tools, approvals, schedules, logs, and runtime steps.
...
System settings include platform branding, plans, API keys, and analytics. Branding settings can affect platform name, logos, colours, email sender details, fonts, and related white-label presentation. Plans control tenant allocation and commercial packaging. API keys support public API access and should be treated like sensitive operational credentials. Analytics settings and pages support platform monitoring and reporting.

Maintenance includes system health, connection checks, scripts, and auth diagnostics. System health reads Hakken's own data health and alert state. Connections asks the outside dependencies and scheduled jobs whether they were contacted recently and whether they need attention. Scripts are maintenance tools and should be run only when the operator understands the script’s purpose and effect. Auth diagnostics support troubleshooting sign-in and account issues. Maintenance tools are not customer-facing everyday workflows; they are operational controls.

## Audit and Login Tracking

Hakken records audit events for many privileged actions, including company create/update/delete, company prompt updates, user create/update/delete, invitation creation and revocation, email template changes, admin login and logout, impersonation changes, super-admin assignment or detachment, and assistant safety refusals. Audit logs help explain who changed what and when.

The header records successful login metadata for signed-in users, including device information and a best-effort IP/location lookup. If the IP lookup fails, Hakken stores a concealed or unknown fallback. Repeated identical login records are throttled to reduce noise. Administrators should use audit and login records as operational evidence, but they should also recognize that location data can be approximate or unavailable.
...
Administration is powerful because it connects to every major Hakken subsystem. Treat changes as operational changes, not cosmetic edits. When in doubt, make the smallest change that solves the problem, verify it with a normal user journey, and leave enough context in audit notes or support records for the next operator to understand what happened.
