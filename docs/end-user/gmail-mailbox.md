...
The Gmail mailbox connector lets Hakken watch one dedicated Gmail account for a
...
the real conversation: received mail, sent replies, and the label Hakken applies
after handling a message.

This is separate from [Emails From Hakken](./emails.md). Those are outbound
...
dedicated address, Hakken reads the conversation, replies when company knowledge
...
- Hakken polls connected mailboxes once a minute.
...
- For normal inbound mail, Hakken reads the whole Gmail thread, searches company
...
- If the sender needs a person, Hakken still sends a holding reply and creates a
  task for follow-up.
- If the sender matches a known customer, the customer Wiki page can be updated
  from the exchange after the reply is sent.

Handled messages are labelled `Hakken` in Gmail. A colleague opening the mailbox
should be able to see which messages Hakken has dealt with.
...
When the mailbox is not connected, choose **Connect mailbox**. Hakken opens
...
When disconnecting, Hakken revokes the token at Google and then removes its
stored token. The Gmail account itself is untouched.

## Human Follow-Up

When Hakken decides a person is needed, it creates a task and sends a holding
reply so the sender is not left with silence. The task includes the sender,
subject, a shortened copy of the latest message, and what Hakken already sent.
...
Hakken stores operational metadata for the mailbox ledger: connector, Gmail
