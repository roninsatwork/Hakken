...
- `/admin/ai/tools/mcp/new`: create a custom connector.
...
Connector install and sync actions are super-admin controlled. Testing a connector records diagnostic state so operators can see whether the connector is untested, passing, or failing.
...
This is intentional for connector scaffolding. Operators should test connector behavior before telling a customer that a connector can complete live external actions.
