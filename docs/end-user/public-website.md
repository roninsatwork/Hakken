...
Audience: sales, support, operators, and customer-facing teams explaining the pre-login Sonae site.

## What The Public Website Is

The public website is the pre-login Sonae experience at `/`. It explains Sonae
...
This page is not the authenticated Sonae dashboard. It does not expose customer
...
The public site currently focuses on one message: Sonae is the production layer
...
- a public navigation bar with the Sonae wordmark, Sign in, and Talk to us
- a mobile navigation drawer with Sign in and Talk to us
- a hero section explaining the product category
- a "What Sonae is" section that breaks the platform into five foundations
- a proof section showing three product examples: Properties, Reports, and
  Posture Studio
- a "Who it is for" section explaining which teams benefit from Sonae
- the name story: Sonae means preparedness
...
logged-in app. It presents Sonae as a foundation for building governed AI
...
- Sonae wordmark, linking back to `/`
- Sign in, linking to `/login`
- Talk to us, using the configured contact destination

Public pages that are still planned, such as Platform, Built on Sonae, Trust &
...
The hero presents Sonae as a way to "Build agent-powered products with
...
The "What Sonae is" section explains five foundations:
...
These are examples of what can be built on Sonae. They should be explained as
...
the normal Sonae sign-in behavior. Users still need the correct account and
...
The footer explains the Sonae name and includes a company credit. The credit
...
fallback URLs. This matters because Sonae can be packaged and deployed in
different contexts.
...
- [Sonae Product Overview](./platform-overview.md)
