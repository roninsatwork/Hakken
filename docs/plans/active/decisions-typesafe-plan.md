# Decisions — the judgments Hakken makes, visible and switchable
...
question ("summarise the platform policy on refunds") through Ask Hakken.
...
- **A Decision** is a named judgment with a plain-English question, a fixed
  answer set, a stakes level, a fallback (the rule Hakken uses today), and a
  record of every run.
...
Decisions job, not the only one, and Hakken must never depend on it. Every
