...
- **Started 2026-07-10.** Item 4 partially done: `AvatarPreview.tsx` deleted (zero consumers); the `legacy-*` `MovementDataFormat` variants stay — stored recordings actively use them. Item 2 first tranche done: `movementRecordedProofManifest.ts` (2,042 lines) and `movementExpansionPreviewAudit.ts` (348 + test) moved to `scripts/movement-debug/lib/` — only the analyze CLI, one analyzer test, and npm script contracts touch them; guard contracts stayed 41/41. Item 3 first two stages done: the page's ~500 lines of pure helpers/thresholds extracted to `replay-lab/_lib/replayLabHelpers.ts`, the Proof Rehearsal QA section extracted to `replay-lab/_components/ReplayProofRehearsalPanel.tsx`, the setup-review + avatar-follow batch sections to `ReplayBatchReviewPanels.tsx`, the recordings/sessions list to `ReplayRecordingList.tsx`, the Current Frame diagnostics column (Start Gate, Avatar Follow, Frame Diagnostics, Game Decision, Frame Flags) to `ReplayCurrentFramePanel.tsx`, and the hidden QA snapshot + Flag Review sections to `ReplayAnalysisReviewSections.tsx`; page 3,505 → 2,018 lines. Every stage re-scored at 0 cells worse.
