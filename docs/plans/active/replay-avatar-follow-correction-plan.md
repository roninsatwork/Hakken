...
1. **Gate truth first.** A supported Replay `review` session is always `review-only` and blocks acceptance; `blocked` remains blocked. Remove the path where a review session with no persisted worst frame becomes `accepted-candidate`.
2. **Tighten rendered error.** Use 0.22 as the general capture-backed lower-body ceiling, retaining 0.12 for active-leg proof, 0.18 for upper body/mirror-side proof, and 0.08 for planted-foot clearance. The headless visual-match heuristic remains diagnostic until it is recalibrated or backed by rendered telemetry; it must not override stronger capture-backed evidence.
3. **Require fresh proof — code complete, recapture pending.** Each capture manifest carries a deterministic SHA-256 fingerprint over the production motion pipeline; the analyzer propagates capture fingerprints into the proof manifest; the gate rejects missing, mixed, or stale fingerprints. The July 9 bundle now correctly blocks as legacy proof.
4. **Fix instructor feet from recorded data — code complete, focused replay proof passed; full refresh pending.** Every confident instructor foot segment now reaches the shared world-space retargeter. The three old motion/contact/knee-lift vetoes are deleted; planted-foot flattening and floor/foot-lock remain the post-retarget contact authority. Side-bend investigation also found that both spine drives put 60% side-bend roll into the hips, vertically splitting otherwise straight legs. Hips now stay level for side bend while spine/chest/upper-chest carry the bend. On the exact user-reported Full Motion Exercises frame 2743, planted-foot clearances improved from `+0.2851/-0.2916` to `-0.0001/-0.0001`, lower error improved from `0.0412` to `0.0285`, and the current-frame failure list cleared. Frames 2781/2782 held both feet within `0.0018` of the floor. No exercise label drives these decisions.
...
6. **Close cross-avatar and live proof.** Re-run the current proof set for VIPE plus unprofiled MoonGirl/Eugenia, persist a compact score comparison, then perform the runbook's short live check only after recorded Replay/Game proof is green.
...
- Remaining: step 4 default-profile collapse review (partially done via the unification comments), instructor-foot decision above, a durable revision-stamped MoonGirl/Eugenia comparison, and the live-path check the runbook requires before client demos (this change touches the live player path too). These are tracked in Acceptance gap closure steps 3, 4, and 6.
