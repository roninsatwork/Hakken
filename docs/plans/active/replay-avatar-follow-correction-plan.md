...
- **Remaining:** the page's component-logic block (optional further split), `movementProofRehearsalEvidence`/`movementNextProofRehearsal` eviction (their remaining consumer is the extracted panel), the `movementCoverageRegistry` decoupling (runtime modules read support status from it — needs a split of runtime switches vs proof bookkeeping), and item 1's `_lib` fold with the guard update.
