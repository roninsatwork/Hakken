...
- **Phase 2 remaining — 2c sweep:** delete now-dead `armTargetComposition`/frontBias/safeZScale computation and rationalize the arm fallback telemetry labels; retire the legacy lower-body aim path (`retarget-legacy-fallback` owner, `movementAvatarAimApplication`, `lowerBodyAimTargets`, `createVrmImageSolverLandmarks` and its `*3.0` scale); rationalize foot ownership (the weakest region, 0.15–0.31 error on goldens); remove `shouldUseRetargetedUpperBody` and related owner flags from the decision types. **2d:** smoothing consolidation (single per-bone slerp table).
