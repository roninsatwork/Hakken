...
| E5 | `updatePiiConfig` stores whatever JSON string it is handed, unread — no validation on the way in, and the read only asserts a type over `JSON.parse`. The repository's own test stores `{ enabled: false, maskCharacter: "#" }` and expects it back, so the stored value can be missing every field the config is supposed to have. | Nothing visible: the screens read the fields they know and ignore the rest. But nothing guarantees the redaction settings a deployment is running are the settings anyone chose. | **Not fixed — raised.** `getPiiConfig` declares `v.any()`, which is honest about what it returns. Validating on the way in changes what the write accepts, which is a product decision rather than a shape. |
...
| E5 (Posture Studio, 2026-08-27) | Three of the eleven movement surfaces are only ever called by a test from the *wrong owner's* side, where they correctly answer with nothing: `listDebugTrackingSessions`, `getDebugTrackingSessions` and the populated branch of `getDebugTrackingSession`. No screen calls any of them at all. Their shapes are therefore checked by the compiler and by nothing at run time. | Nothing today — no screen reads them. But a declaration nothing exercises is a declaration nobody is checking, which is the finding this whole effort keeps making. | **Not closed.** Covering them means adding an owner-side case to the frozen suite, which is wider than the permission given. Recorded here rather than done quietly. |
...
- **Anthony's side, as of 2026-08-27.** Two things wait on him, both raised
  rather than taken: whether the privacy-masking config should validate what it
  accepts on the way in (a change to what the Save button allows, so a product
  decision), and whether the three Posture Studio surfaces that no screen and no
  owner-side test reaches should get one — covering them widens the frozen suite
  beyond the permission he gave. Neither blocks anything. Nothing is pushed: 35
  commits sit on `dev` and he decides when.
