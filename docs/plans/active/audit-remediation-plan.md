...
| E3 broad analytics reads | **partial — the two largest are closed** (2026-08-26: bc143b7c4) | The register's own "largest unprotected read on the platform" — two 20,000-row overview scans — plus a 10,000-row model read and the money view's four read-then-filter counts now read one row past their cap and say so on screen in both languages. Three entries left the register under its stale-entry rule; the mid-sized ceiling fell 126 to 118. **What remains is the six reads in `convex/analytics.ts` that name the snapshot pipeline redesign in their own reasons** — those are a pipeline, not a cap, and are new work rather than a touch-up. Probes broken and watched to fail |
| E4 return validators beyond the client surface | **partial — the harm is closed, the count is ratcheted** (2026-08-26: 25561ddb1) | Measured honestly: 72 client-callable declarations declare a shape and 455 do not; internal builders are 4 and 393. Writing 455 validators is not a session's work and the count was never the harm — a handler posting a whole database row is. Fourteen did; four are fixed and ten are frozen. Two were real: `users.tokenIdentifier` reached any colleague in the same company, `workflows.webhookSecret` reached the workflow editor. `return-shape-drift.test.ts` freezes the population shrink-only and names the ten. **What remains is 455 shapes to write** — mechanical, large, and now unable to grow while it waits |
