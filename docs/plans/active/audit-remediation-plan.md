...
|---|---|---|
| Client-callable, no declared shape | 362 *(really 365 — see "Found in passing": the guard could not see `governanceAction`)* | **In scope.** These are the surfaces a browser can call, and the ones where an undeclared shape can put something on the wire that was never meant to travel |
