...
signature of load sensitivity rather than a broken test; a fourth and a fifth on the same
file during E5, same message, each passing alone immediately and on the next
full run — the fifth took two of that file's tests rather than one): `wikiDistillActions.test.ts` (during WP04) and
...
| E5 | With the row rule parsing rather than matching, `invites.ts` came into view: `getPendingInvites` and `getInvitesByCompany` returned whole `invitations` rows, and that table carries a `token`. Nothing reads it — the sign-in path finds an invitation by email — so it was a secret-shaped value with no reader at all. | Four admin screens received an invitation token for every pending invite: the team page, the user directory, System Administrators and a company's user list. None of them displays it. | Narrowed through a new `toClientInvite` and declared, with the screens' own row type moved onto the client shape so the compiler agrees. Both narrowings probed. |
| E5 | `toolConnectorOAuthConnections` rows carry `state` — the value that completes a pending OAuth connection, made unguessable by `buildOAuthState` for exactly that reason. The connector screen received it for every connection, and `beginConnectorOAuth` returned it outright. Nothing reads either: the screen follows `authorizationUrl` and never touches `state`. | Nothing visible. An administrator's browser held the value that finishes somebody's pending connector authorisation, on a screen with no use for it. | Narrowed through `toClientOAuthConnection`, and `beginConnectorOAuth` stops returning it. Probed — and the first probe did *not* fail, because every existing test reached the connector screen while no connection existed. A test that seeds a pending connection was written first, and then the probe failed. |
| E5 | The platform-wide analytics board hands every person the workspace they belong to, and the shape written for it did not name that field — so `getGlobalAnalytics` refused its own answer. No test reached that surface, so the whole suite stayed green. | The Running Costs screen showed "This page could not be loaded" for a super admin. Caught in the browser on the first look, not by a test. | Shape corrected, and a test written that reads the board with a person who has a workspace and a person who has none, so both branches of that field are real. Probed by narrowing it back. |
| E5 | `threads.widgetAccessTokenHash` — the hash a widget session authenticates against — reached all five chat-log surfaces. `chat.ts` narrowed it out of a person's own history and called that drift exposure, because a personal history holds no widget threads. The admin chat logs are the opposite: widget-originated threads are what they exist to show. | Every administrator opening the chat logs received the authentication hash of every widget conversation listed. Nothing on the screen reads it. | Narrowed through `toClientAdminThread`, declared, and probed — after a fixture was written that carries a hash, because none did and the first probe passed. |
| E5 | `workflows.webhookSecret` reached the workflow list. `list` and `get` were narrowed through `toClientWorkflow` when this was last closed; `getPaginatedWorkflows` was not — and it is the one the list screen calls. Nothing reached it in a test either. | A workflow with a webhook trigger handed its secret to the browser on every page of the workflow list. | Narrowed and declared, with a test that creates a webhook workflow, asserts the secret exists in the database, and asserts it is absent from the list. Probed. |
| E5 | The five model-catalogue syncs returned the whole formatted catalogue — hundreds of records — to a screen that types the result `unknown` and never opens it. | Nothing visible; a large payload on every Sync press, read by nobody. | They report `{ synced }` now. The catalogue is already in the database by the time the action returns. |
| E5 | **A `v.record()` return declaration proves nothing.** `convex-test`'s `validateValidator` has no `case "record"`, so the switch falls through and the value is never checked — keys, value types and extra fields all pass. Three declarations written during E5 used one. | Nothing yet. But a record-shaped declaration reads as a guard and is not one, which is the exact failure mode four audits have found here. | All three replaced with objects naming every key: the fifteen purge pipelines twice, and the seven tool-shelf groups. Each is now probed and fails. The point is recorded here because the next person to write a return shape will reach for `v.record` too. |
| E5 | `updatePiiConfig` stored whatever JSON string it was handed, unread — no validation on the way in, and the read only asserted a type over `JSON.parse`. A switch missing from the stored value arrived as `undefined`, and `redactPII` reads each switch as a plain truthiness test, so a switch that had gone missing behaved exactly like one somebody had turned off — with the screen drawing it off, which reads as a decision rather than a gap. The retention screen beside it has always validated its own config. | Nothing visible today; all five switches are on. But masking could have stopped silently on a deployment with no way to tell that from a choice. | **Fixed 2026-08-27 on Anthony's instruction**, modelled on `normalizePurgePipelineConfigForUpdate`: the write refuses bad JSON and a switch that is not a switch, drops anything unrecognised, and stores five complete switches; the read fills anything absent from the defaults, which covers settings stored before the refusal existed. `getPiiConfig` declares the real shape instead of `v.any()`. Six tests, and both halves probed by removing them. |
| E5 | `apiKeys.keyDigest` — the value the server compares an incoming API key against — reached the API keys screen for every key it lists. The only code that reads it is that comparison. | An administrator's browser held the digest of every live API key on the platform. Not the key itself, and not usable to authenticate, but a credential-derived value on a screen that never shows it. | Narrowed through `toClientApiKey`, declared, and probed. |
| E5 | `convex/auth.test.ts` mocked the whole of `@convex-dev/auth/server` with a two-line object. That worked by accident: nothing in that file's import graph reached the package's other exports. Adding a return shape to `authEvents.ts` pulled `convex/schema.ts` into the graph — the schema spreads `authTables` from that package — and nine tests failed at once on a module that has nothing to do with authentication events. | Nothing a user sees. But a mock that blanks a module is a test that fails for reasons unrelated to what it checks, and the next person meets it as a mystery. | The mock keeps the real module and replaces only `convexAuth`, which is how the other mock in the same file was already written. |
| E5 (re-check, 2026-08-27) | **The drift guard's builder list was missing `governanceAction`,** so three client-callable declarations were never counted by anything: `evidencePack.produce`, `personalData.produceSubjectAccess` and `personalData.retainedExceptions`. The population the handover stated as 362 was really 365, and the "done" figure of 11 was really 14. Found by checking the guard's hand-written set against the builders `tenantFunctions.ts` actually exports — not by the guard, which read as clean throughout. | Nothing a user sees; the evidence pack and the subject-access answer both work. But a hand-written list of names is the thing that goes quietly out of date, and this one had. | All three declared. The guard now reads `tenantFunctions.ts` and fails on any export that is neither classified as a builder nor listed as not one — probed both ways: `governanceAction` removed from the list, and a new builder added to the module without being classified. |
| E5 (Posture Studio, 2026-08-27) | Three of the eleven movement surfaces were only ever called by a test from the *wrong owner's* side, where they correctly answer with nothing: `listDebugTrackingSessions`, `getDebugTrackingSessions` and the populated branch of `getDebugTrackingSession`. No screen calls any of them at all, so the browser could not stand in for a test either. Their shapes were checked by the compiler and by nothing at run time. | Nothing today — no screen reads them. But a declaration nothing exercises is a declaration nobody is checking, which is the finding this whole effort keeps making. | **Closed 2026-08-27 on Anthony's instruction.** One owner-side case in `movements.test.ts` reads all three with a real session behind them, and pins the narrowing the list already performs: `samplesJson` is the raw capture blob and the list sends an 800-character preview instead. All three probed. |
...
|---|---|---|
| Client-callable, no declared shape | 362 *(really 365 — see "Found in passing": the guard could not see `governanceAction`)*, **now 0** | **In scope.** These are the surfaces a browser can call, and the ones where an undeclared shape can put something on the wire that was never meant to travel |
...
- **Anthony's side, as of 2026-08-27.** Nothing is outstanding. Both open
  decisions came back the same day and both are done: the privacy-masking config
  validates what it accepts, and the three uncovered Posture Studio surfaces have
  a test. Nothing is pushed: the commits sit on `dev` and he decides when.
- **The earlier open question is closed.** It was whether "Real Auth
