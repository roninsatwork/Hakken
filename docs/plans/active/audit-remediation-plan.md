...
| E5 remaining return shapes | **in progress** | 362 client-callable declarations with no shape, plus 5 short-form handlers the classifier could not read — those five are done. Scheduled on 2026-08-26 because Sonae is the baseline every product is forked from: debt left here is inherited by every fork and cannot easily be taken back. 10–15 days, one file at a time, ceiling ratcheted down after each. The 393 internal-only declarations are deliberately out of scope. **Progress: 362 → 11, and the eleven are `convex/movements.ts`, which is excluded permanently.** `agentSkills.ts` (23), `users.ts` (11), `invites.ts` (3), `agentEvalFixtures.ts` (15), `knowledge.ts` (13), `wikiPages.ts` (13), `companies.ts` (12), `aiTools.ts` (11), `aiModels.ts` (11), `agents.ts` (10), `companySkills.ts` (10), `salesData.ts` (9), `companyEvals.ts` (8), `salesDataCustomers.ts` (8), `salesDataResearch.ts` (8), `analytics.ts` (7), `companyMemories.ts` (7), `aiModelsActions.ts` (6), `chatAdmin.ts` (6), `scheduler.ts` (6), `workflows.ts` (6) and nine smaller files are done — the memory-candidate, suggestion, reflection, feedback, purge, widget, research-job and platform-settings surfaces, then `agentRuns.ts` (7) and the tool servers. **All five short-form handlers the classifier could not read are declared.** The last two were taken out of size order because declaring their shapes found live surfaces still handing back whole rows — a sign-in identity and an invitation token — and the guard that reported zero of those could not see the shape they were written in. The row rule parses now, its blindness is probed four ways, and the 34 real offenders it found are frozen and shrink-only. See "Found in passing" |
