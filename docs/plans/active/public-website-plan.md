...
2. ~~**Template boundary**~~ — removed 2026-08-09 with the template-generation machinery (`template.manifest.json`, `scripts/build-template.mjs`, `src/template-boundary.test.ts`). The repo no longer generates a stripped starter; it is cloned whole. Nothing needs registering in a manifest. The `// template:remove:start` markers still in shared files are inert and can be left alone or cleared as files are touched.
