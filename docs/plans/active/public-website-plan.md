...
2. **Template boundary** (`template.manifest.json`, `src/template-boundary.test.ts`): the repo generates a white-label template that strips verticals. Register `src/app/(public)/showcase/properties`, `.../reports`, `.../studio` under their verticals' `paths` in the manifest **in the same commit** that creates them, and fence vertical references in shared public files with `// template:remove:start <vertical>` / `// template:remove:end` markers (works in TSX with `{/* */}` full-line comments too).
...
- [~] **Phase 2 — Home.** Built 2026-07-30, all sections live: product-as-hero (real dashboard screenshot in tilted browser frame, floating governance chips at parallax depths, entrance timeline, scroll-settle), capability marquee, three slide-over product panels (properties: animated link→grid; reports: self-assembling chart with count-up and risk chips; studio: real screenshot), GSAP-pinned layer-stack scrub, agent-run timeline with approval pause, 備え story, closing CTA. Screenshots captured from the `next-dev-e2e-auth` fixture build via Playwright into `public/marketing/` (hero-app.png, studio-app.png; more in session scratchpad). KNOWN ITEMS: fixture branding "Sonae E2E / E2E Company Admin" visible in screenshots (re-capture with a presentable workspace name later); mobile pass not yet done; `.next` may hold an E2E-mode build — run a plain `npm run build` before any real deployment. **CHECKPOINT: awaiting Anthony's reaction to the built home page.**
