...
The public site today is one screen of template boilerplate that says nothing about Hakken. The product is finished and no longer hidden, so the site must become a suite of publicly visible pages that explain Hakken to a cold visitor, showcase the product and our UX/UI craft, and get people to make contact. Anthony wants to be able to send anyone the URL and have them understand what Hakken is.
...
| Company credit | Hakken is owned by Anthony personally, NOT by Ronins. The only acceptable credit is **"Powered by Ronins"** (footer). Never "a Ronins product". |
| "To be prepared" / naming story | Hakken (備え) means "to be prepared / preparedness" in Japanese — that is why the name was chosen. Tell this story properly: full version on the Built on Hakken page, echoed as the closing beat of the home page. |
...
Hakken's translation of that, agreed with Anthony:
...
- `/showcase` **Built on Hakken** (index) — opens with the full 備え naming story, then the three products.
...
CTAs on public pages route to `/contact` (never into the app). Nav: Platform, Built on Hakken, Trust & Security, Contact, plus Sign in (→ `/login`) and a "Talk to us" primary button.
...
8. Root layout metadata is leftover junk ("Hakken - Protocol" / "Hakken Living Dossier") — replace during Phase 8, plus per-page metadata and OG tags.


## Current state of the home page (2026-07-30, end of the design session)

Order: hero → What Hakken is → demos lead-in → three demo panels → Who it's for → 備え → CTA → footer.
...
- **Nav/footer links** are trimmed to routes that exist. Platform, Built on Hakken and Trust & Security return as those pages ship.
