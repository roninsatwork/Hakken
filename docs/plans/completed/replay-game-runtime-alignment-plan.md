...
Status: active, scope-expanded, and product-blocked. The shared post-input runtime is substantially aligned, proof identity and eight schema-v2 comparison boundaries now fail closed, and schema-v3 Deep Capture adds a ninth exact dense-fusion boundary. A browser-captured and saved 147-frame schema-v3 packet now passes fresh Replay, mounted Game, exact nine-boundary comparison, and the ordered targeted tier. Product acceptance remains blocked by the fixed representative and all-nine schema-v3 sets, the full behaviour matrix, supported lower-tier-device evidence, browser-visible ordinary Game review, and the accepted reviewer manifest.
...
- [x] Make the shared passive neutral-prefix setup the normal Game path behind one Start action, retry weak initial prefixes until trustworthy setup evidence arrives, and keep manual calibration visible only in the debug lane.
...
- [x] Add a capability profile that distinguishes ordinary pose capture from required Deep Capture commissioning. Ordinary saves remain schema v2; the query-free canonical `/demos/movement-capture/deep` route locks commissioning to schema v3, while `/demos/movement-capture?deepCapture=1` remains a compatible alias. Standard capture is now visibly labelled schema v2 and links directly to the locked route instead of silently resembling Deep Capture.
...
- [x] Immediately rerun that exact packet through Replay and mounted Game and compare acquisition/setup provenance plus final rendered telemetry. On 2026-07-20 the saved 147-frame packet `px7de1rfvvgprnkqh4x8qqnv1d8axpm9` rendered 147/147 frames in Replay, processed 147/147 in mounted Game with zero skips, and passed all nine boundaries across 81 active frames.
...
The next concrete action is to produce the fixed three-recording schema-v3 representative set (`Full Motion Exercises`, `Full Spinal Flow`, and `Spins`) from retained source evidence, run the representative mounted-Game/Replay tier, and keep every absent channel honestly absent. Do not ask Anthony to repeat a recording while preserved source material or automated capture can supply the proof lane. The passing 147-frame targeted packet commissions capture/save and the shared runtime, but it is not relabelled as the complete movement matrix or the final nine-recording set.
...
5. Only after the start gate, preflight, profile, and capture channels are green, record one schema-v3 commissioning packet at the query-free locked route `/demos/movement-capture/deep`, preflight it with `npm run movement:replay-game:deep-commissioning-eligibility -- --recording-id <id> --export <export> --fail-on-ineligible`, and run `npm run movement:replay-game:deep-commissioning-proof -- --recording-id <id>` from its immutable recording ID.
...
## 2026-07-20 Saved Deep Capture And Resource-Lifecycle Evidence

- The authenticated browser capture route saved `Automated Deep Capture Proof 2026-07-20` as immutable recording `px7de1rfvvgprnkqh4x8qqnv1d8axpm9`: schema v3, 147 frames, dense evidence on 147/147 frames, genuine left-hand evidence on 36 frames, genuine right-hand evidence on 62 frames, and a recoverable local packet backup. The fresh export inventory found this packet eligible and retained all ten older ineligible recordings unchanged.
- The first fresh commissioning comparison exposed six consecutive left-hand divergences at the Game start boundary. Replay had processed six readiness frames beyond the 60-frame setup prefix while mounted Game began from the shorter history. The shared live-motion hook now retains controlled pre-start frames until the first active frame and never resumes collection after start or pause. Focused lifecycle tests pass, and the same immutable packet now passes Replay/mounted Game comparison with matching identities, zero checksum divergences, 81 compared active frames, and all nine required boundaries.
- The ordered targeted Deep Capture tier now passes for this saved packet. The next proof blocker is the fixed three-recording representative schema-v3 set; the final all-nine set and accepted device/reviewer evidence remain open.
- The MediaPipe warning was a real resource-lifecycle defect: the capture evidence builder read `MPMask` data without closing the per-result mask. A failing ownership test proved the leak before the fix. The mask is now closed in `finally`, including error paths. A 148-frame authenticated browser run produced dense evidence on 148/148 frames and zero `MPMask ... not closed` warnings; other reported MediaPipe/WebGL diagnostics were retained rather than suppressed.

## 2026-07-19 Deep Capture Route Evidence

- The freshly saved `New Full Exercises` packet (`px7fmtzet5smehkp94vd56ahcn8atf2g`, 2,453 frames) was preserved and inventoried from a fresh Convex export. The strict gate correctly rejected it as schema v2 with no Deep Capture profile, face/blendshape channel, or complete bilateral refined-hand evidence; it was not relabelled or silently promoted.
- `/demos/movement-capture/deep` is now the query-free canonical schema-v3 route. Authentication redirects preserve that path, the page exposes `data-capture-profile="schema-v3-deep-capture"`, and the visible banner states `Capture profile locked: schema-v3 Deep Capture` before recording.
- Ordinary `/demos/movement-capture` remains available for schema-v2 capture but now exposes `data-capture-profile="schema-v2-standard"`, an explicit warning that it is not Deep Capture, and a direct switch to the locked route.
- Focused route, authentication, page-state, and acquisition-drift tests pass. Authenticated browser proof followed the redirect to the exact query-free route, visibly rendered the locked profile, and proved the standard-page switch returns to that same route.
- The saved packet also exposed a second acquisition gap: its coarse Face Landmarker produced zero face/blendshape frames. Deep Capture face refinement formerly required that same coarse face crop. Capture now derives a conservative fallback face ROI from trustworthy bilateral Pose nose/eye/ear/mouth evidence plus shoulder scale, runs the existing native-resolution Face Landmarker only inside that crop, and publishes face evidence only when the real model returns its complete 478-point result. Unit coverage passes for far, edge, partial/rotated and weak/one-sided rejection cases. On 2026-07-19 the authenticated browser-local fixture using the saved `Screenshot 2026-07-18 at 18.00.26.png` passed the real MediaPipe scale-gap lane: the 20%-scale full-frame detector returned zero faces, `pose-face-fallback` supplied only the native crop, and the crop refiner returned 478 image landmarks, 10 iris points, two gaze vectors, 52 blendshapes and a 16-value facial transform. The rendered overlay and explicit PASS verdict were browser-visible; no synthetic face points were introduced.
- 2026-07-19 dense-runtime correction supersedes the earlier 300 ms carry and `384x216` low-tier notes above. Measured browser evidence showed that configuration produced zero anchors in five of six representative scenarios. Valid dense evidence is now carried for at most 1,200 ms with explicit decayed `temporally-tracked`/occluded provenance, just beyond the one-second inference backoff, and the low tier uses viable `640x360` medium-quality inference at a 300 ms cadence. An authenticated synthetic Deep Capture page run recorded 143 schema-v3 frames with 143 valid dense frames and genuine bilateral hand evidence, then passed Replay and mounted Game with zero source or rendered frame skips and all nine shared comparison boundaries across 77 active frames.
- The normal Game entry screen now exposes one Score, one Start action, one camera preview and one plain-language readiness message. Start arms indefinitely while the shared passive setup and full-body gate wait for trustworthy evidence, then enters a three-second countdown; it does not require a separate posture/spine judgement. The active instruction is a large, high-contrast centred banner designed to remain readable after the player steps back: missing lower-body evidence says `Step back until your head and both feet are visible`, complete body evidence says `Perfect — stay there`, and the banner explicitly says no second Start press is required. Detection produces a browser-visible large `3, 2, 1` countdown before automatic start. Posture check-in, calibration moments, Posture Sync, Spine and Guided Preview are absent from the player lane but remain available in debug repair routes. The automatic setup now slides past a weak initial 60-frame prefix instead of becoming permanently stuck. Focused UI/setup/readiness coverage passes, and the authenticated browser-visible `New Full Exercises` Game route proved one Score, one enabled Start, zero player-facing posture/debug controls, a disabled armed state with actionable distance-readable guidance, the synthetic full-body countdown transition, and zero browser errors.
