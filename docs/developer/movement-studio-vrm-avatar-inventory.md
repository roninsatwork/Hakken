...
| Arm target selection | 1045-1072, 1526-1572 | `movement-decision` plus `avatar-application` | Partial | `movementAvatarArmTargetDecision.ts` now resolves per-side arm readiness from shoulder/elbow/wrist confidence, with the hand landmark standing in for a weak wrist. Arms themselves are owned by the rest-mapped segment retarget, so there is no separate vector-aim path left to gate: `movementAvatarArmApplication.ts` only decides how to degrade when the retarget could not solve a side, dispatching between hold-last-good and relax and wiring that choice to VRM bones. `vrmRigging.ts` owns relaxed-arm, last-good, and hand-neutral fallback target/execution wrappers, including direct relaxed-arm, last-good, and hand-neutral bone application. Renderer still resolves VRM bones and supplies last-good storage. |
...
| `resolveMovementAvatarStudioDecision` vs `resolveMovementAvatarReplayDecision` | Compatibility wrappers for parity harnesses and migration tests. | Collapsed into one `resolveMovementAvatarPipelineDecision` in `movementAvatarPipelineDecision.ts`, which takes a `sourceOrigin` of `"studio"` or `"replay"`. The two old names survive only as local shims in `movementAvatarMotionFrameInput.test.ts`; runtime paths should use `MovementSourceFrame -> MovementMotionFrame`. |
