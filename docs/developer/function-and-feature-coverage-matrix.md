...
| Knowledge management | global, company, agent, thread, and website/file/manual knowledge routes | `convex/knowledge.ts`, `convex/knowledgeActions.ts`, `knowledgeDocuments`, `knowledgeChunks`, storage | `docs/end-user/knowledge-management.md`, `docs/developer/knowledge-management.md`, `docs/developer/upload-and-knowledge-policy.md` | Complete pair | Some docs are concise; review after ingestion, embedding, or retrieval changes. |
...
| Embedded widgets | `/w/[widgetId]`, `/sandbox/[widgetId]`, global widget, company widget | `convex/widgets.ts`, `widgets`, `threads`, `messages`, upload policy | `docs/end-user/embedded-widgets.md`, `docs/end-user/widget-handoff-and-troubleshooting.md`, `docs/developer/embedded-widgets.md` | Complete pair | Refresh when contact gating, domain checks, or widget upload policy changes. |
...
| Agent runtime, runs, approvals, memory, evals, and observability | `/admin/agents/[id]/runs`, evals, memory, logs, observability, approvals | `convex/agentRuns.ts`, `convex/agentRuntime.ts`, `convex/agentEvalFixtures.ts`, `convex/agentMemories.ts`, `convex/agentLogs.ts`, `convex/agentRunFeedback.ts`, `convex/agentImprovementSuggestions.ts` | `docs/end-user/agent-operations-and-review.md`, `docs/developer/agent-runtime-operations.md`, `docs/developer/run-observatory.md`, active agent plans | Complete pair plus active plans | Needs refresh as Agent Observability, AI Checks, Agent Autonomy, and Run Properly plans land. |
...
| Workflow automation and schedules | `/admin/workflows`, workflow detail, executions, schedules | `convex/workflows.ts`, `convex/workflowRuntime.ts`, `convex/workflowEngine.ts`, `convex/scheduler.ts`, `src/lib/convexHttpActionsUrl.ts`, `workflows`, `workflowExecutions`, `workflowExecutionSteps`, `schedules` | `docs/end-user/workflow-automation.md`, `docs/developer/workflow-automation.md`, `docs/developer/workflow-runtime-internals.md` | Complete pair | Keep approval nodes, public triggers, webhook origin display, schedule behavior, and runtime internals current. |
...
| System health, alerts, diagnostics, retention, and purges | `/admin/health`, `/admin/auth-diagnostics`, audit log detail, purge settings/history | `convex/analyticsCron.ts`, `convex/platformAlertService.ts`, `convex/purges.ts`, `convex/auditLogs.ts`, `purgeHistory`, `analyticsDailySnapshots`, `auditLogs` | `docs/end-user/health.md`, `docs/end-user/system-health-and-maintenance.md`, `docs/end-user/operational-diagnostics-and-retention.md`, `docs/developer/system-health-and-platform-alerts.md`, `docs/developer/data-retention-and-purges.md`, `docs/developer/auth-diagnostics.md` | Complete pair | Keep alert recipients, health signal thresholds, and purge semantics current. |
...
| Developer setup, architecture, deployment, and extension | repo config, scripts, deployment workflow, starter/template docs | package scripts, `template.manifest.json`, app architecture docs | `docs/developer/getting-started.md`, `architecture.md`, `backend.md`, `frontend.md`, `deployment.md`, extension and starter docs | Covered broadly | Several short docs should be reviewed when setup, deployment, or template boundaries change. |
...
| `agentRuns.ts` | `getPublicRunStatusInternal`, `getReplayExecutionContextInternal`, `getLatestStepIndexInternal`, `getRunExecutionStateInternal`, `getPendingApprovals`, `getPendingApprovalCount`, `decideApproval`, `createRunInternal`, `createPublicAgentRunInternal`, `updateRunStatusInternal`, `recordRunUsageInternal`, `appendStepInternal`, `insertToolCallInternal`, `insertApprovalInternal`, `getApprovalResumeContextInternal`, `recordApprovedToolResultInternal`, `getApprovalExpiryConfig`, `updateApprovalExpiryConfig`, `expireStalePendingApprovals`, `getSettlementAfterDecisionInternal`, `continueRunAfterApprovalInternal`, `completeApprovalResumeInternal` |
| `agentRuntime.ts` | `runAgentObjective`, `continueAgentObjective`, `generateAgentResponse`, `runTriggeredAgentObjective`, `resumeApprovedToolCall`, `resumeAfterRefusedToolCall`, `executeAgentNode` |
...
| `ai.ts` | `generateSonaeResponse`, `transcribeAudio`, `generateThreadTitle` |
| `aiActionRequests.ts` | `reserve` |
| `aiModels.ts` | model list/search/count, provider controls, global/company defaults, runtime resolution, embedding resolution, enforcement, sync upsert, backfills, pricing |
...
| `analyticsCron.ts` | analytics backfills, validation, system health, daily snapshots, platform alerts, snapshot wiping |
...
| `knowledge.ts` and actions | document list/pagination/quality/inspection/retrieval, upload/save/delete/retry/repair, website queueing, chunks, ingestion actions, re-embedding |
...
| `widgets.ts` | global/primary widget reads, public widget config, widget upload, widget thread creation |
...
| `workflowRuntime.ts` | start workflow, execute node, resume approval step |
...
| Tools, connectors, widgets, and webhooks | `toolConnectors`, `toolConnectorTestLogs`, `toolConnectorSecretRefs`, `toolConnectorOAuthConnections`, `aiTools`, `agentTools`, `widgets`, `webhookDeliveries` |
| Knowledge and chat | `knowledgeDocuments`, `knowledgeChunks`, `threads`, `messages` |
...
6. Short-doc review: revisit concise guides when their implementation changes,
