...
Audience: agents and engineers documenting every Hakken feature, route, backend function family, schema area, and operational command.

## Purpose

This matrix is the durable coverage control for documenting the whole Hakken app.
...
| Property research and Rightmove/Apify flow | `/app/properties/information`, search, scraped data, detail, logs | optional module (not included in this copy), optional module (not included in this copy), `convex/apify.ts`, `convex/webhooks.ts`, `properties`, `apifyRuns` | optional module (not included in this copy), optional module (not included in this copy), `docs/plans/active/rightmove-agent-execution-plan.md` | Complete pair plus active plan | The Search page now queues the Rightmove Agent. Refresh again when Apify run records link visibly back to agent run detail or the active plan is retired. |
| Sales and board reports | `/app/reports/information`, `/app/reports` | optional module (not included in this copy), optional module (not included in this copy), `salesReports`, knowledge docs | optional module (not included in this copy), optional module (not included in this copy) | Complete pair | Refresh when report history, manual regeneration, source selection, or document export changes. |
| Sales Data workspace | `/app/[workspace]/spreadsheet-import`, `/app/[workspace]/import-data`, `/app/[workspace]/customers`, `/app/[workspace]/customers/[account]`, `/app/[workspace]/opportunity-report`, legacy `/app/sales-data` redirects | optional module (not included in this copy), optional module (not included in this copy), optional module (not included in this copy), optional module (not included in this copy), optional module (not included in this copy), optional module (not included in this copy), optional module (not included in this copy), sales data tables | optional module (not included in this copy), optional module (not included in this copy), sales-data active plans | Complete pair plus active plans | Refresh when workspace routing, import mapping, reset scope, research jobs, market discovery, or opportunity-report workflow changes. |
...
| Temporary Posture Studio and movement demo | `/demos/movements`, replay lab, play, capture, benchmark, proof routes | optional module (not included in this copy), movement debug scripts, `movements`, `movementDebugSessions` | optional module (not included in this copy), optional module (not included in this copy), movement docs and active movement plan | Frozen | Do not expand except under explicit user approval and movement guardrails. |
