...
The collection pipeline starts from the DataForSEO Planner's and Collector's own agent schedules and then runs on its own; each company's Collection schedule is a setting the Planner reads, never an alarm. Everything about its shape follows from one fact: **DataForSEO charges when a task is posted, not when its result is read.** A duplicate is an invoice line, not an untidy log.
...
**The hourly sweep is the watchdog, not the driver.** `seo-collection-sweep` runs through `jobLedger` like every other job. Since 2026-09-23 it neither plans nor sends — only the Planner and Collector agents do. Its duties, each its own transaction and each paging through what it finds within a page budget and six minutes in all (collection reliability plan, 3.3): return claims older than `SEO_CLAIM_TIMEOUT_MS` that never reached the send, and fail those that did; fetch every submitted task out an hour whose pingback never came, spaced ten a second, and give up those past `SEO_RESULT_TIMEOUT_MS`; close settled cycles; restart work lists that stopped being written, from the step they reached; file again answers recorded and never filed; close Planner and Collector runs that died without saying so; clear expired answers; and retire old cycles of every end state with their lines and reports. Hourly and not minutely: it exists to catch failure. Its last result is shown on each company's Collection runs screen as well as on Health.
