...
- Recording upload transport uses the same permission and byte-limiting gateway. Movement capture, compression, packet semantics and rendered motion remain unchanged.

## Policy Source

Backend metadata enforcement lives in `convex/utils/uploadPolicy.ts`. All upload
issuers return the single-use gateway in `convex/uploadHttp.ts`; ownership,
transactional quotas and cleanup live in `convex/uploadReservations.ts`.
See [Controlled file uploads](./convex-environment-variables.md#controlled-file-uploads)
for the permission lifecycle, precise quotas and release checks.
...
New uploads are byte-counted while streaming, before they can become attachable.
Saving mutations check the uploader, company and purpose before metadata validation;
knowing a storage ID is not permission to use or delete it. A failed mutation rolls
back its database/storage changes, so deletion inside a throwing validator is not
a cleanup guarantee. The gateway deletes files whose finalization fails, and the
bounded cleanup job removes expired/unclaimed uploads after their claim window.
...
- `convex/uploadReservations.test.ts`, `convex/uploadHttp.test.ts` and `convex/utils/uploadStream.test.ts` for permissions, ownership, quotas, cleanup, HTTP failure paths and the full 50 MiB streaming boundary.
- `convex/uploadIngressGuard.test.ts` to prevent direct storage-upload bypasses.
