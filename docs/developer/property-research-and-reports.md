...
This document covers Sonae's implemented property research and board-reporting surfaces. It is written for engineers and agents who need to maintain the Rightmove extraction flow, company-scoped property inventory, extraction status screens, and the AI-generated sales report dashboard. The related user-facing documentation is `docs/end-user/property-research-and-reports.md`.
...
`src/app/(dashboard)/app/properties/search/page.tsx` is the entry point for Rightmove collection. It renders a two-step form: a Rightmove search URL and a numeric max-property limit. Client-side validation only checks that the trimmed URL contains `rightmove.co.uk`; security validation is performed again on the backend. The numeric control allows values from 10 through 1000 in the browser. Submission calls the Convex action `api.apify.startRightmoveScrape` with `listUrls` containing the single entered URL and `maxProperties`. Success clears the URL and shows a translated success message. Failures are surfaced inline through `getErrorMessage`, not through native browser dialogs.
...
`convex/apify.ts` owns the external extraction actions. `startRightmoveScrape` first reads the current user through `api.users.getMe`; unauthenticated callers are rejected. It validates every submitted URL with `validateSafeUrl(url, "Rightmove Scraper")` to reduce SSRF risk. It requires `APIFY_API_TOKEN`, `CONVEX_SITE_URL`, and `APIFY_WEBHOOK_SECRET`. The action starts Apify actor `jKpgGfgRfzrGgEMa8` with full property details, price history, Apify proxy, the requested list URL, and the requested property limit. It registers a webhook to `${CONVEX_SITE_URL}/apify-webhook` with the shared secret header and a small JSON payload containing run, status, actor, and dataset identifiers.

After the actor starts, `startRightmoveScrape` records the run through `internal.webhooks.recordRunStart`. The run is associated with `startedBy: user._id` and `companyId: user.companyId`. It then schedules `internal.apify.pollRunStatus` after 60 seconds. The poller reads Apify status. Terminal statuses trigger `api.apify.syncRunStatus`; non-terminal statuses reschedule another poll after 60 seconds. This watchdog prevents the UI from relying solely on inbound webhooks.

`syncRunStatus` is a public action used by both the logs screen and poller. It requires `APIFY_API_TOKEN`, loads the Apify run, updates the local run to pending for non-success states, and fetches the dataset only when Apify reports `SUCCEEDED`. Successful datasets are passed to `internal.webhooks.storeRightmoveData`.
...
`convex/bola.test.ts`, `convex/properties.test.ts`, and `convex/salesReports.test.ts` are the most important regression tests for tenant isolation. They verify cross-company properties cannot be read or deleted by the wrong admin, search and counts are company-scoped, run logs are company-scoped, admin-only run queries are protected, unsafe scrape URLs are rejected, and company admins only read their own latest report.
...
```bash
npm run test:run -- convex/properties.test.ts convex/webhooks.test.ts convex/salesReports.test.ts convex/bola.test.ts
```
