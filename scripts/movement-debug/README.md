...
Add `--capture-guide-out tmp/movement-replay-lab/current-upper-body-standing-broad-capture-guide.md` after deciding existing candidates are not enough. The guide writes the explicit broad capture protocol plus the post-capture analyzer, focused Game visual plan/capture/review, and merged readiness-audit commands. Use `--capture-label <label>` to customize the scratch output names.
