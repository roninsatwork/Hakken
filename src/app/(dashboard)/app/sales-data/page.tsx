...
function SalesDataTables() {
  ...
  const pagination = useCursorPagination(
    JSON.stringify([tab, currentImportId, search, customerType, accountName, groupName])
  );
  ...
  const selectTab = (next: Tab) => {
    ...
    setAccountName(null);
    setGroupName(null);
    router.replace(`/app/sales-data?tab=${next}`, { scroll: false });
  };
  ...
  const categories = useQuery(
    ...
    tab === "categories" ? { paginationOpts, ...searchArg } : "skip"
  );
  const interest = useQuery(
    api.salesData.listAreasOfInterest,
    tab === "interest" ? { paginationOpts, ...searchArg } : "skip"
  );
  const frequencies = useQuery(
    api.salesData.listFrequencies,
    tab === "frequency" ? { paginationOpts, ...searchArg } : "skip"
  );
  ...
  const hasFilters = Boolean(customerType || accountName || groupName);
