        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-2">
              <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.email.label')}</label>
              <input
                type="email"
                required
                value={inviteEmail}
                ...
                className={`w-full px-4 py-3 bg-black/20 border rounded-[12px] text-[14px] text-foreground outline-none transition-all placeholder:text-muted ${sendError
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.role.label')}</label>
          ...
          {isSuperAdmin && (
            <div className="flex flex-col gap-2">
              <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.company.label')}</label>
              <select
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border-dim/50 pb-4">
            ...
            <div className="flex items-center gap-3">
              {saveSuccess && <span className="text-[#10b981] text-[12px] font-medium flex items-center gap-1.5 animate-in fade-in"><CheckCircle2 className="w-3.5 h-3.5" /> {t('actions.synchronized')}</span>}
              <button
                type="button"
              ...
              >
                ...
                {isSaving ? t('actions.saving') : t('actions.saveTemplate')}
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="flex flex-col gap-6 pt-2">
              <div className="flex flex-col gap-2">
                <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.subject.label')}</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.headline.label')}</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.body.label')}</label>
                <textarea
                  rows={5}
                  value={formData.body}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[11px] font-mono tracking-widest text-muted uppercase">{t('fields.cta.label')}</label>
                <input
                  type="text"
            ...
            <div className="p-8 lg:p-10 bg-[#050505] border border-white/5 rounded-[24px] shadow-2xl flex items-center justify-center">
              <div className="w-full max-w-[380px] bg-[#121212] border border-[#1A1A1A] rounded-[24px] p-8 flex flex-col items-start transition-all">
                ...
                <h1 className="text-[20px] font-semibold text-white tracking-tight mb-3 leading-tight">{formData.headline || t('preview.headlineEmpty')}</h1>

                <p className="text-[#A3A3A3] text-[13px] leading-relaxed mb-8 whitespace-pre-wrap">{formData.body || t('preview.bodyEmpty')}</p>

                <button type="button" className="bg-white text-black font-medium px-5 py-2.5 rounded-[10px] text-[13px] pointer-events-none">
                  {formData.ctaText || t('preview.ctaEmpty')}
                </button>
