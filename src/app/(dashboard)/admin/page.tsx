...
export default function AdminDashboard() {
  ...
  const getAggregationLabel = (agg: string) => {
    if (agg === "month") return "Monthly Scaling Traces";
    if (agg === "week") return "Weekly Scaling Traces";
    return "Daily Expenditure & Engagement Traces";
  };
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <MetricBlock 
              ...
              sub="Rolling Global MAU"
              delay={0.1}
            />
            <MetricBlock 
              icon={Activity}
              title="Period Compute Executions"
              value={(data.aggregates.totalTokens ?? 0).toLocaleString()}
              sub={`${(data.aggregates.totalMessages ?? 0).toLocaleString()} messages inferred`}
              delay={0.2}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-[-12px]">
            <MetricBlock 
              icon={Users}
              title="Period Active Context"
              value={(data.aggregates.activeUsers ?? 0).toLocaleString()}
              sub="Identities active in this timestamp"
              delay={0.3}
            />
            <MetricBlock 
              icon={PoundSterling}
              title="Gross Logistic Burn"
              value={`£${(data.aggregates.totalCostGBP ?? 0).toFixed(4)}`}
              sub="Server compute execution cost"
              delay={0.4}
            />
          </div>
                 ...
                 {data.timeline && data.timeline.length > 0 ? (
                   <ResponsiveContainer width="100%" height="100%">
                     <AreaChart data={data.timeline}>
                       ...
                       <Tooltip 
                         ...
                         formatter={(value: any, name: any) => [
                            ...
                            name === 'cost' ? 'Estimated AI Cost' : 'Global Messages'
                         ]}
                       />
                       <Area 
                         yAxisId="left"
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-muted opacity-50 relative z-10 bottom-8">
                      ...
                      <span className="text-[12px] uppercase font-mono tracking-widest">No Chronological Data</span>
                    </div>
                 )}
               </div>
            </motion.div>
               ...
               >
                  <div className="px-6 py-5 border-b border-border-dim bg-background/30 flex flex-col gap-1.5">
                     <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-[14px] font-bold text-foreground">Top Performing Tenants</h2>
                     </div>
                  </div>
                  <div className="flex flex-col">
                     {data.topCompanies.length === 0 ? (
                        <div className="p-8 text-center text-secondary text-sm font-mono tracking-widest uppercase opacity-50">Empty Vector Space</div>
                     ) : (
                        data.topCompanies.map((c, i) => (
                           <div key={c.id} className="flex justify-between items-center px-6 py-4 border-b border-border-dim/50 last:border-0 hover:bg-foreground/[0.03] transition-colors">
                              ...
                              <div className="flex flex-col items-end min-w-[80px]">
                                 ...
                                 <span className="text-[10px] text-secondary/60 font-mono tracking-widest uppercase">{c.messages.toLocaleString()} inf</span>
                              </div>
                           </div>
                        ))
                     )}
               ...
               >
                  <div className="px-6 py-5 border-b border-border-dim bg-background/30 flex flex-col gap-1.5">
                     <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-[14px] font-bold text-foreground">Top Individual Initiators</h2>
                     </div>
                  </div>
                  <div className="flex flex-col">
