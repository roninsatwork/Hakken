...
export default function CompanyWidgetPage() {
  ...
  const widgets = useQuery(api.widgets.getWidgetsByCompany, { companyId });
  ...
  const [themePrimaryColor, setThemePrimaryColor] = useState("#000000"); // Assuming brand black/dark as default
  ...
  const handleCreateOrUpdate = async () => {
    ...
    try {
      ...
      await saveWidget({
        ...
        themeGreeting,
        themePrimaryColor,
        // agentId: we'd connect an agent selector here later, omitting for now to use fallback logic
      });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-6 w-full pb-12">
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-secondary mt-1 tracking-wide">
            Configure the embeddable chat widget for the company website.
          </p>
        </div>
      </header>
      ...
      {widgets === undefined ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 px-6 text-center border border-border-dim/50 border-dashed rounded-[16px] bg-foreground/[0.02]">
          ...
          <h3 className="text-sm font-medium text-foreground mb-1">No Widget Configured</h3>
          <p className="text-[13px] text-secondary max-w-sm mb-6">
            Create a secure embeddable widget to allow external users to interact with this company's knowledge base.
          </p>
          <button
