...
import NewCompanyRulePage from "./page";
...
const COMPANY_ID = "company_1234567890";

vi.mock("next/navigation", () => ({
  useRouter: () => ({ push: pushMock }),
  useParams: () => ({ id: "company_1234567890" }),
}));
...
const INSTRUCTION = "e.g. Inform the user that Sonae does not handle refunds...";
const SUBMIT = "Save Rule";

describe("NewCompanyRulePage", () => {
  ...
  /**
   * The company id is the whole point of this screen: the same form without one
   * creates a rule that applies to every customer on the platform.
   */
  it("creates the rule against the company whose page it was opened from", async () => {
    renderWithProviders(<NewCompanyRulePage />);
  ...
  it("returns to the company's rules once the rule is created", async () => {
    renderWithProviders(<NewCompanyRulePage />);
