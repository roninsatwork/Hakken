...
type AgentRuleEditContentProps = {
  agentId: Id<"agents">;
  ruleId: Id<"aiRules">;
  rule: Doc<"aiRules">;
  renderSafetyWarning: (trigger: string, instruction: string) => ReactNode;
};
...
export default function AgentRuleEditContent({
  agentId,
  ruleId,
  rule,
  renderSafetyWarning,
}: AgentRuleEditContentProps) {
  ...
  return (
    <div className="flex flex-col gap-4 w-full pb-8">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 flex-1 relative max-w-4xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 ml-1">
            {(["LOW", "NORMAL", "HIGH", "CRITICAL"] as const).map((priority) => (
              <WriteButton
                ...
                className={`flex flex-col items-start gap-1 p-3 rounded-[10px] border transition-all text-left ${priority === form.priority ? selectedClasses[priority] : `border-border-dim bg-transparent text-secondary ${priorityClasses[priority]}`}`}
              >
                <span className="text-[12px] font-bold tracking-widest uppercase font-mono">{t(`priority.levels.${priority}`)}</span>
              </WriteButton>
            ))}
          </div>
        </section>
        ...
        {renderSafetyWarning(form.trigger, form.instruction)}
