...
export default function AgentDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (agent === undefined) {
    return <div className="p-8 text-secondary">Loading agent...</div>;
  }
  if (agent === null) {
  ...
  const tabs = [
    { label: 'Dashboard', href: `/admin/agents/${agentId}`, icon: LayoutDashboard },
    { label: 'Settings', href: `/admin/agents/${agentId}/settings`, icon: Settings },
    { label: 'Knowledge', href: `/admin/agents/${agentId}/knowledge`, icon: Library },
    { label: 'Prompt', href: `/admin/agents/${agentId}/system-prompt`, icon: Terminal },
    { label: 'AI Rules', href: `/admin/agents/${agentId}/rules`, icon: Scale },
    { label: 'Integrations', href: `/admin/agents/${agentId}/integrations`, icon: Cpu },
    { label: 'I/O Schemas', href: `/admin/agents/${agentId}/schemas`, icon: Code2 },
    { label: 'Logs', href: `/admin/agents/${agentId}/logs`, icon: FileText },
  ];
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
             ...
             <div className="flex flex-col gap-0.5">
               ...
               <p className="text-[13px] text-secondary mt-1 max-w-[500px] truncate">{agent.description || 'No description provided.'}</p>
             </div>
          </div>
