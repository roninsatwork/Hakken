...
import type { LayoutNode, WikiMapLayout } from "./wikiMapLayout";
...
const WIDTH = 1000;
const HEIGHT = 700;
const EMPTY_LAYOUT: WikiMapLayout = { nodes: [], edges: [] };
...
export function WikiMapScreen({
  ...
  const [calculatedLayout, setCalculatedLayout] = useState<{
    rows: typeof rows;
    layout: WikiMapLayout;
  } | null>(null);
  const layout =
    calculatedLayout !== null && calculatedLayout.rows === rows
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void import("./wikiMapLayout").then(({ runForceLayout }) => {
      if (!cancelled) setCalculatedLayout({ rows, layout: runForceLayout(rows) });
    });
