...
export default function GlobalWidgetPage() {
  ...
  useEffect(() => {
    if (widget) {
      ...
      setThemePrimaryColor(widget.themePrimaryColor || "#4f46e5");
  ...
  return (
    <>
      <div className="flex flex-col gap-6 w-full pb-12">
        {/* Header Block constraints natively matched to Sonae styles */}
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-[13px] text-secondary mt-1 tracking-wide max-w-2xl">
              Configure the system-wide embeddable chat widget operated by the global AI protocol. Customizations apply instantly across all permitted integrations.
            </p>
          </div>
          {widget && (
              <button
          ...
          <div className="flex flex-col items-center justify-center py-24 px-6 text-center border border-border-dim/50 border-dashed rounded-[16px] bg-foreground/[0.02]">
            ...
            <h3 className="text-sm font-medium text-foreground mb-1">No Global Protocol Connected</h3>
            <p className="text-[13px] text-secondary max-w-sm mb-6">
              Create a secure embeddable widget to allow external users to interact with the global knowledge base and intercept logic.
            </p>
            <button
              onClick={handleCreateOrUpdate}
              disabled={isSaving}
                  ...
                  <div className="flex flex-col gap-6 p-6 md:p-8 rounded-[20px] bg-sidebar/40 border border-border-dim shadow-sm backdrop-blur-xl">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        <div className="flex flex-col gap-2">
                            <label className="text-[13px] font-semibold text-secondary">Identify Label</label>
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-background border border-border-dim rounded-[10px] px-4 py-2.5 text-[14px] text-foreground focus:outline-none focus:border-brand transition-colors" placeholder="e.g. ARI" />
                        </div>
                        <div className="flex flex-col gap-2">
                                ...
                                <div className="flex items-center gap-4 bg-background border border-border-dim rounded-[10px] p-3">
                                    <div className="w-12 h-12 rounded-full overflow-hidden border border-border-dim/50 flex-shrink-0 bg-sidebar/50">
                                        {logoPreviewUrl ? (
                                            ...
                                            <Bot className="w-full h-full p-3 text-secondary" />
                                        )}
                                    </div>
