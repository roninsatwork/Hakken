...
export default function AICostsDashboard() {
  ...
  const getAggregationLabel = (agg: string) => {
    ...
    return "Daily Expenditure Traces";
  };
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-3">
            ...
            AI Logistics & Cost Profiling
          </h1>
          <p className="text-[13px] text-secondary tracking-wide max-w-xl">
            Real-time interception of Vertex AI Token utilization mathematically bounded against configurable temporal sequences.
          </p>
        </div>
        ...
        >
          ...
          <div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <MetricBlock 
                ...
                sub={`Input: ${(analytics.periodInputTokens ?? 0).toLocaleString()} / Output: ${(analytics.periodOutputTokens ?? 0).toLocaleString()}`}
                delay={0.1}
              />
              <MetricBlock 
                icon={Bot}
                title="Period Interactions"
                value={(analytics.periodProcessed ?? 0).toLocaleString()}
                sub="Platform-wide executed operations"
                delay={0.2}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-[-12px]">
            <MetricBlock 
              ...
              sub="Averaged against chronological users"
              delay={0.3}
            />
            <MetricBlock 
              icon={MessageSquare}
              ...
              sub="Averaged against chronological threads"
              delay={0.4}
            />
          </div>
