...
/**
 * One tended page (wiki plan, phase 3): what Sonae currently believes about
 * a customer, the corrections staff have pinned above it, and the walkable
 * history of every rewrite. Editing the body files a revision exactly like a
 * machine rewrite; the pinned layer belongs to people alone and survives
 * everything the machine does.
 */
export default function WikiPageDetailPage() {
  const t = useTranslations("aiPages.detail");
  const params = useParams();
  const pageId = params.pageId as Id<"wikiPages">;
  const detail = useQuery(api.wikiPages.getPageDetail, { pageId });
  const editContent = useMutation(api.wikiPages.editPageContent);
  const pin = useMutation(api.wikiPages.pinCorrection);
  const unpin = useMutation(api.wikiPages.unpinCorrection);
  ...
  if (detail === null) {
    return (
      <div className="flex flex-col items-center gap-4 py-24 text-secondary">
        ...
        <Link href="/admin/ai/pages" className="text-[13px] text-brand hover:underline">
          {t("back")}
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col gap-6 pb-12 w-full">
        ...
        <ul className="flex flex-col gap-2">
          {detail.pinnedCorrections.map((correction) => (
            ...
            >
              ...
              <AdminWriteButton
                onClick={() => void run(() => unpin({ pageId, pinnedAt: correction.pinnedAt }))}
                disabled={isSaving}
      ...
      <section className="flex flex-col gap-3 rounded-[16px] border border-border-dim bg-card/40 p-5">
        ...
        <div className="flex items-center gap-3">
          <AdminWriteButton
            onClick={() => void run(() => editContent({ pageId, content: draft ?? "" }))}
