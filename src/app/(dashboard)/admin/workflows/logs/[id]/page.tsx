      ...
      return (
          <div className="flex flex-col items-center justify-center p-20 gap-4 text-center">
              ...
              <div className="flex flex-col gap-1">
                 <h2 className="text-xl font-bold text-foreground">Log Not Found</h2>
                 <p className="text-secondary text-sm">This execution log may have been purged or does not exist.</p>
              </div>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[13px] text-secondary mt-1">Detailed breakdown of the workflow autonomous transaction.</p>
          </div>
        </div>

        <div className="flex flex-col items-end gap-1">
             <span className="text-[10px] text-muted tracking-[0.2em] uppercase font-mono">Status</span>
             {exec.status === "SUCCESS" && <span className="flex items-center gap-1.5 text-green-500 font-bold text-[14px] uppercase"><CheckCircle2 className="w-5 h-5" /> SUCCESS</span>}
             {exec.status === "FAILED" && <span className="flex items-center gap-1.5 text-red-500 font-bold text-[14px] uppercase"><XCircle className="w-5 h-5" /> FAILED</span>}
             {exec.status === "RUNNING" && <span className="flex items-center gap-1.5 text-blue-500 font-bold text-[14px] uppercase"><Loader2 className="w-5 h-5 animate-spin" /> RUNNING</span>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
          <div className="flex flex-col gap-2 p-5 rounded-[16px] bg-sidebar/30 border border-border-dim/50">
             <span className="text-[10px] text-muted uppercase font-mono tracking-widest flex items-center gap-2"><Play className="w-3 h-3" /> Trigger Type</span>
             <span className="text-[15px] font-semibold text-foreground">{exec.triggerType}</span>
          </div>
          <div className="flex flex-col gap-2 p-5 rounded-[16px] bg-sidebar/30 border border-border-dim/50">
             <span className="text-[10px] text-muted uppercase font-mono tracking-widest">Workflow Target</span>
             <span className="text-[15px] font-semibold text-brand">{exec.workflowName}</span>
          </div>
          <div className="flex flex-col gap-2 p-5 rounded-[16px] bg-sidebar/30 border border-border-dim/50">
             <span className="text-[10px] text-muted uppercase font-mono tracking-widest">Started At</span>
      ...
      <div className="flex flex-col gap-4 relative">
         <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-bold tracking-widest uppercase text-foreground">Graph Checkpoint Output</span>
         </div>
         ...
         <div className="relative w-full rounded-[16px] overflow-hidden bg-[#0A0A0A] border border-border-dim/40 shadow-inner group">
             ...
             <pre className="p-6 pt-16 overflow-x-auto text-[13px] font-mono text-[#D4D4D4] leading-relaxed custom-scrollbar">
                 {exec.state ? (
                     ...
                     <span className="text-muted/50 italic">No output payload registered for this sequence.</span>
                 )}
             </pre>
         </div>
      </div>
