          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border-dim text-[11px] uppercase tracking-[0.1em] text-muted">
                ...
                <th className="px-4 py-3 font-medium">{t('table.timestamp')}</th>
              </tr>
            </thead>
            <tbody>
              <AnimatePresence>
                    ...
                    {paginatedLogs.map((exec) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          {exec.status === "SUCCESS" && <span className="flex items-center gap-1.5 text-green-500 font-medium text-[12px] uppercase"><CheckCircle2 className="w-4 h-4" /> {t('status.success')}</span>}
                          {exec.status === "FAILED" && <span className="flex items-center gap-1.5 text-red-500 font-medium text-[12px] uppercase"><XCircle className="w-4 h-4" /> {t('status.failed')}</span>}
                          {exec.status === "RUNNING" && <span className="flex items-center gap-1.5 text-blue-500 font-medium text-[12px] uppercase"><Loader2 className="w-4 h-4 animate-spin" /> {t('status.running')}</span>}
                        </td>
