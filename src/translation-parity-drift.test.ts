...
describe('the Italian catalogue is translated, not copied', () => {
  ...
  const IDENTICAL_ON_PURPOSE: ReadonlyMap<string, string> = new Map([
    ...
    ['admin.collectionRuns.figures.nextRunHint', 'placeholders and a separator only: no words to translate'],
    ['admin.collectionRuns.detail.money.legend', 'placeholders and a separator only: no words to translate'],
    ['sites.pageRecord.crawl.seconds', 'a number and the unit symbol for seconds, written the same in both languages'],
    ['sites.pageRecord.crawl.kilobytes', 'a number and the unit symbol for kilobytes, written the same in both languages'],
