...
export function SubNavItem({ label, isActive, onClick, href, badge, badgeAtLimit }: { label: string, isActive: boolean, onClick: () => void, href?: string, badge?: number, badgeAtLimit?: boolean }) {
...
export function NavItem({ icon: Icon, label, isActive, hasChildren, isOpen, onToggle, onClick, children, href }: NavItemProps) {
  const content = (
    <>
      {isActive && (
        <motion.div
 ...
 */
export class OptionalNavSection extends Component<
  { children: React.ReactNode },
  { failed: boolean }
> {
  state = { failed: false };
