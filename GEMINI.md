...
- **MANDATORY - ALWAYS RESTART SERVERS**: If you stop the Next.js local development server, Convex backend server, or any other service for any reason (e.g., to run a build sweep or compile), you **MUST ALWAYS** immediately restart it as a background task (`source ~/.zshrc && npm run dev` / `source ~/.zshrc && npm run convex:dev`) before concluding your turn. Leaving the user's local environment offline is strictly unacceptable.
