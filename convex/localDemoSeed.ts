...
export const seed = publicMutation({
  ...
  returns: v.object({ company: v.object({ companyId: v.id("companies"), name: v.string(), action: v.union(v.literal("created"), v.literal("updated")) }), users: v.array(v.object({ email: v.string(), userId: v.id("users"), action: v.union(v.literal("created"), v.literal("updated")) })), provider: v.object({ providerKey: v.string(), action: v.union(v.literal("created"), v.literal("updated")) }), models: v.array(v.object({ modelId: v.string(), action: v.union(v.literal("created"), v.literal("updated")) })), defaults: v.array(v.object({ useCase: v.string(), action: v.union(v.literal("created"), v.literal("updated")) })), tool: v.object({ toolId: v.id("aiTools"), modelName: v.string(), handlerMapping: v.string(), action: v.union(v.literal("created"), v.literal("updated")) }), agent: v.object({ agentId: v.id("agents"), name: v.string(), templateId: v.string(), action: v.union(v.literal("created"), v.literal("updated")) }), knowledge: v.object({ documentId: v.id("knowledgeDocuments"), title: v.string(), action: v.union(v.literal("created"), v.literal("updated")) }), toolBinding: v.object({ action: v.union(v.literal("created"), v.literal("existing")) }), evalFixtures: v.object({ fixtureIds: v.array(v.id("agentEvalFixtures")), action: v.union(v.literal("created"), v.literal("existing")) }) }),
