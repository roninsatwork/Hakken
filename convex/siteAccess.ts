...
export function groupOwner(site: Site): Doc<"companyWebsites"> | null {
  return isTrackedHold(site.hold) ? site.pair : site.hold;
}
...
/**
 * The website whose lists — searches and questions — the site's pages are
 * read against. An owned site's own; a competitor's, the owned site it is
 * watched against, because that is what the company chose to measure it on.
 */
...
/** The most rows one page of a Sites table may ask for: a screen shows fifteen. */
