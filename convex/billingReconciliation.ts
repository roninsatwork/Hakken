...
export async function reconcileAccount(ctx: Pick<ActionCtx, "runQuery" | "runMutation">, stripe: Stripe, account: Doc<"billingAccounts">) {
  ...
  const owned = all.data.filter(s => s.metadata.sonaeBillingAccount === account._id);
  ...
  if (current.length > 1 || (selected && selected.metadata.sonaeBillingAccount !== account._id)) {
  ...
  if (account.attemptId && subscription.metadata.sonaeCheckoutAttempt === account.attemptId &&
  ...
  const supported = offer && subscription.livemode === (account.mode === "live") && objectId(subscription.customer) === account.customerId &&
    subscription.metadata.sonaeBillingAccount === account._id && subscription.items.data.length === 1 && !subscription.items.has_more &&
