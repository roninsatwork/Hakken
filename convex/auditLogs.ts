...
import { publicMutation, publicQuery, superAdminMutation } from "./tenantFunctions";
import {
  AUDIT_PURGE_ACTION,
 ...
 *
 * Two things this does beyond deleting.
 *
 * It leaves its own summaries alone, so the record that the trail was shortened
 * cannot itself be shortened away next month. And when the whole run is done it
 * writes one entry saying how much went, how old it was, and under which rule —
 * a trail that can be quietly cut down, with nothing saying it was, fails a
 * review on its own.
 *
 * Paged by cursor rather than by repeatedly taking the oldest rows. Taking the
 * oldest works only while everything found is deletable; the moment some rows
 * are exempt, the same exempt rows come back every batch and the run never
 * ends.
 *
...
export const executePurge = internalMutation({
  args: {
    retentionDays: v.number(),
    // Carried across batches. A purge that spans several transactions still has
    // to report one figure at the end rather than one per batch.
    cursor: v.optional(v.string()),
    removedSoFar: v.optional(v.number()),
    oldestRemovedAt: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    ...
    let removed = args.removedSoFar ?? 0;
    let oldestRemovedAt = args.oldestRemovedAt;

    for (const log of batch.page) {
      if (isAuditPurgeRecord(log.actionType)) continue;

      oldestRemovedAt =
        oldestRemovedAt === undefined ? log.timestamp : Math.min(oldestRemovedAt, log.timestamp);
    ...
    if (!batch.isDone) {
      await ctx.scheduler.runAfter(1000, internal.auditLogs.executePurge, {
    ...
    // A run that removed nothing is not worth a record. Writing one every hour
    // the cron fires would bury the runs that did remove something.
    if (removed === 0) return;

    await ctx.db.insert("auditLogs", {
      // No actor. Retention is the platform acting on its own schedule, and
      // naming whoever last edited the setting as the person who deleted the
      // records would be a fiction.
      actionType: AUDIT_PURGE_ACTION,
      entityType: "auditLogs",
      entityId: "RETENTION",
      timestamp: Date.now(),
      metadata: buildAuditPurgeSummary({
