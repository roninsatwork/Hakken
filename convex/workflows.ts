...
export const handleWebhook = httpAction(async (ctx, request) => {
  ...
  try {
    ...
    // Enforce webhook secret strictly via headers (x-hakken-secret), completely omitting URL param checking to avoid credential leak via proxy logs.
    const secretHeader = request.headers.get("x-hakken-secret");
