 ...
 *
 * A plain === returns at the first differing character, and that timing
 * difference is measurable over enough requests. This existed as four
 * byte-identical private copies (apiKeys, webhooks, workflows,
...
export function validateSafeUrl(url: string, context: string = "URL"): void {
  try {
    ...
    if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
      throw appError("INVALID_INPUT", `SSRF Prevention: Invalid protocol via ${context}`);
    }

    if (host === "localhost" || host === "metadata.google.internal" || host === "metadata.azure.com") {
      throw appError("INVALID_INPUT", `SSRF Prevention: Restricted internal host denied via ${context}`);
    }
    ...
    if (host.includes(":")) {
      ...
      if (isLoopback || isPrivate) {
         throw appError("INVALID_INPUT", `SSRF Prevention: Private IPv6 address denied via ${context}`);
      }
      if (normalizedHost.includes("::ffff:")) {
         throw appError("INVALID_INPUT", `SSRF Prevention: IPv4-mapped IPv6 address denied via ${context}`);
      }
    }
    ...
    if (ipv4Private.test(host)) {
        throw appError("INVALID_INPUT", `SSRF Prevention: Private IPv4 address denied via ${context}`);
    }
    
    // 3. Reject non-standard representations of IPs (octal, hex, int)
    if (/^\d+$/.test(host)) {
        throw appError("INVALID_INPUT", `SSRF Prevention: Numerical IP representation denied via ${context}`);
    }
    
    if (/^0x[0-9a-f]+$/i.test(host) || host.includes(".0x")) {
        throw appError("INVALID_INPUT", `SSRF Prevention: Hexadecimal IP representation denied via ${context}`);
    }
    ...
    if (/^0[0-7]+(\.0[0-7]+)*$/.test(host) || host.includes(".0")) {
        ...
        if (parts.some(p => p.length > 1 && p.startsWith("0") && !p.includes("x"))) {
            throw appError("INVALID_INPUT", `SSRF Prevention: Octal IP representation denied via ${context}`);
        }
    }
