...
export const getAllUsers = tenantQuery({
  args: {},
  returns: v.array(clientUserValidator),
  handler: async (ctx) => {
    const { user: caller } = ctx;
    if (!caller || !caller.role) throw appError("UNAUTHORIZED", "Unauthorized");
    ...
    if (caller.role === "SUPER_ADMIN" && !caller.impersonatingCompanyId) {
      return (await ctx.db.query("users").order("desc").take(1000)).map(toClientUser);
...
export const getUserById = tenantQuery({
  args: { id: v.id("users") },
  returns: v.union(clientUserValidator, v.null()),
  handler: async (ctx, args) => {
    const { user: caller } = ctx;
    ...
    if (caller.role === "SUPER_ADMIN" || caller._id === args.id) {
       return toClientUser(targetUser);
    }
